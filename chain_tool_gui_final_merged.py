...
def prompt_filter_ui():
    ...
    topic = st.text_input("Filter by topic")
    ...
    filtered = [
        ...
        (not tag or tag in c["tags"]) and
        (not topic or topic.lower() in c["topic"].lower()) and
        (not intent or c["intent"] == intent) and
        (not difficulty or c["difficulty"] == difficulty)
    ]

    for c in filtered:
        st.markdown(f"### {c['scenario']}")
        for i, t in enumerate(c["chain"]):
            st.markdown(f"**Turn {i+1}:** {t}")
        st.markdown(f"**Tags:** {', '.join(c['tags'])}")
        st.markdown(f"**Topic:** {c['topic']}")
        st.markdown(f"**Intent:** {c['intent']}")
        st.markdown(f"**Difficulty:** {c['difficulty']}")
        st.markdown("---")
